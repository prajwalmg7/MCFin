...
contract Stakingpool is Pausable {
 ...
 constructor(MCHToken _mchtoken, MCFToken _mcftoken)  {
  ...
  function Approvestake(uint amount) external {
    ...
    stakedShares[msg.sender] = stakedShares[msg.sender].add(calcShares(msg.sender));
  ...
  function unstake() external  {
    ...
    stakedShares[msg.sender] = stakedShares[msg.sender].sub (calcShares(msg.sender));
   ...
   function calcShares(address user) internal returns(uint) {
     ...
     return  stakedShares[user] = stakedShares[user].add(shares);
